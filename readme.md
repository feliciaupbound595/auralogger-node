...
| ---------------- | -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `init`           | —              | Interactive setup. Prompts for missing values, then prints a **copy/paste dotenv block** (up to 5 lines: `AURALOGGER_PROJECT_TOKEN`, `AURALOGGER_USER_SECRET`, `AURALOGGER_PROJECT_SESSION`, `NEXT_PUBLIC_AURALOGGER_PROJECT_TOKEN`, `VITE_AURALOGGER_PROJECT_TOKEN`) and two code snippets (`**Auralog`** for browser + `**AuraLog`** for server).                 |
| `server-check`   | —              | Sanity-check **server-side** logging connectivity from your machine/CI environment. Uses `AURALOGGER_PROJECT_TOKEN` + `AURALOGGER_USER_SECRET` (from env or prompt) and sends a single test log.                                                                                                                                                                     |
| `client-check`   | —              | Sanity-check **client-style** logging connectivity. Uses the same project token (env or prompt) and sends a single test log using the browser/client path (no server secret in a browser bundle).                                                                                                                                                                     |
...
| `get-logs`       | `[filters...]` | Fetch and print logs; filters use grammar below. Uses `AURALOGGER_PROJECT_TOKEN` + `AURALOGGER_USER_SECRET` (env or prompt). If styles aren’t configured (`**AURALOGGER_PROJECT_STYLES**` or public equivalents), it will resolve them automatically for this run so output is nicely formatted.                                                                       |
